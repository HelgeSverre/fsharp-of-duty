    ...
    member _.Handle(events: GameEvent list) (blood: Vector3) =
        for event in events do
            ...
            | Dismembered(_, position, cut) ->
                for index in 0..15 do
                    let angle = float32 index * 2.39996f
                    let spray = MathEx.normalizedOrZero (cut.Impulse + Vector3(MathF.Cos angle, 0.4f + float32 (index % 3) * 0.2f, MathF.Sin angle))
                    addPuff position (spray * (1.3f + float32 (index % 4) * 0.45f)) (Vector4(blood, 0.92f)) (14.0f + float32 (index % 4) * 4.0f) 0.72f -8.0f
