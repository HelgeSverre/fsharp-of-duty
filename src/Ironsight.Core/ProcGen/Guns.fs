...
module Guns =
    ...
    let meshForPose name pose =
        match name with
        | "M134 Minigun" ->
            // The barrel cluster turns about the bore; everything else is static.
            let axis = 0.06f
            let spun =
                minigunBarrels
                |> MeshGen.transform (
                    Matrix4x4.CreateTranslation(0.0f, -axis, 0.0f)
                    * Matrix4x4.CreateRotationZ pose
                    * Matrix4x4.CreateTranslation(0.0f, axis, 0.0f))
            MeshGen.union [| minigunBody; spun |]
    ...
    /// Whether a weapon's viewmodel changes every frame and has to be
    /// re-uploaded rather than cached.
    let animated name = name = "M134 Minigun" || name = "Bow" || name = "Harpoon Gun"
